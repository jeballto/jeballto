...
struct ImageConcurrencyUtilityTests {
  ...
  // Rendezvous rather than sleeping: if the gate ever serialized across distinct keys,
  // neither task could observe the other's signal and the test fails on timeout.
  @Test
  func operationCacheRunsDistinctKeysConcurrently() async throws {
    let cache = KeyedOperationGate()
    ...
    try await withThrowingTaskGroup(of: Void.self) { group in
      group.addTask {
        try await cache.withExclusiveAccess(for: "pull:sha256-first") {
          await firstEntered.signal()
          await secondEntered.wait()
        }
      }
      group.addTask {
        try await cache.withExclusiveAccess(for: "pull:sha256-second") {
    ...
    #expect(await firstEntered.hasBeenSignalled())
    #expect(await secondEntered.hasBeenSignalled())
  }
