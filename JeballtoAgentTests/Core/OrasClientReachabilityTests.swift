...
private final class ReachabilityCancellationProbe: @unchecked Sendable {
...
private final class SuspendedReachabilityURLProtocol: URLProtocol {
  nonisolated(unsafe) static var probe: ReachabilityCancellationProbe?
...
private func makeSuspendedReachabilitySession() -> URLSession {
  let config = URLSessionConfiguration.ephemeral
  config.protocolClasses = [SuspendedReachabilityURLProtocol.self]
...
struct OrasClientReachabilityTests {
  ...
  @Test
  func checkRegistryReachablePreservesTransportCancellation() async throws {
    let client = makeOrasClient()
    await #expect(throws: CancellationError.self) {
      try await StubURLProtocolGate.shared.run(handler: { _ in throw URLError(.cancelled) }) {
        try await client.checkRegistryReachable(
          registryHost: "registry.example.com",
          insecure: false,
          session: makeStubSession()
  ...
  @Test
  func checkRegistryReachableCancelsSuspendedTransport() async throws {
    let client = makeOrasClient()
    let probe = ReachabilityCancellationProbe()
    SuspendedReachabilityURLProtocol.probe = probe
    defer { SuspendedReachabilityURLProtocol.probe = nil }
    let session = makeSuspendedReachabilitySession()
    defer { session.invalidateAndCancel() }
    let task = Task {
      try await client.checkRegistryReachable(
        registryHost: "registry-cancellation.example.com",
        insecure: false,
        session: session
      )
    }
