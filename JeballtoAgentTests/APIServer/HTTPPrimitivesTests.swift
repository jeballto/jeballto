...
struct HTTPPrimitivesTests {
  ...
  func contentLengthRejectsMalformedValues() throws {
    ...
    let missing = decoder.contentLength(fromHeaderString: "Host: example.test")
    let valid = decoder.contentLength(fromHeaderString: "Host: example.test\r\nContent-Length: 42")
    let malformed = decoder.contentLength(fromHeaderString: "Content-Length: nope")
    let negative = decoder.contentLength(fromHeaderString: "Content-Length: -1")
    let explicitlyPositive = decoder.contentLength(fromHeaderString: "Content-Length: +1")
    let duplicate = decoder.contentLength(
      fromHeaderString: "Content-Length: 1\r\nContent-Length: 1"
    )
    let unsupportedTransferEncoding = decoder.contentLength(
  ...
  func oversizedHeadersReturn431WithOrWithoutTerminator() throws {
    ...
    let unterminated = try #require(
      decoder.headerLimitResponse(accumulatedCount: 65540, headerEnd: nil)
    )
    let terminated = try #require(
      decoder.headerLimitResponse(accumulatedCount: 70000, headerEnd: 65537)
    )

    #expect(unterminated.statusCode == 431)
    #expect(terminated.statusCode == 431)
    let allowed = decoder.headerLimitResponse(accumulatedCount: 65539, headerEnd: nil)
  ...
  func parserRejectsBodiesThatDoNotExactlyMatchContentLength() throws {
    ...
    let decoder = HTTPRequestDecoder()
    let shortResponse = try #require(decoder.decode(shortBody).requestFailureResponse)
    let extraResponse = try #require(decoder.decode(extraBody).requestFailureResponse)
  ...
  func parserRejectsEncodedPathAliasesAndFragments() throws {
    ...
    let decoder = HTTPRequestDecoder()
    #expect(try #require(decoder.decode(encodedHealth).requestFailureResponse).statusCode == 400)
    #expect(try #require(decoder.decode(fragmentedHealth).requestFailureResponse).statusCode == 400)
    #expect(try #require(decoder.decode(authorityAlias).requestFailureResponse).statusCode == 400)
    if case .success(let request) = decoder.decode(encodedQuery) {
  ...
  func parserRejectsMalformedOrAmbiguousQueryParameters(_ target: String) throws {
    ...
    let response = try #require(HTTPRequestDecoder().decode(requestData).requestFailureResponse)
  ...
  func bearerAuthenticationSchemeIsCaseInsensitiveAndRequiresOneOrMoreSpaces() {
    #expect(HTTPRequestDispatcher.bearerToken(from: "Bearer token") == "token")
    #expect(HTTPRequestDispatcher.bearerToken(from: "bearer   token") == "token")
    #expect(HTTPRequestDispatcher.bearerToken(from: "BEARER token") == "token")
    #expect(HTTPRequestDispatcher.bearerToken(from: "Bearer") == nil)
    #expect(HTTPRequestDispatcher.bearerToken(from: "Bearer token extra") == nil)
    #expect(HTTPRequestDispatcher.bearerToken(from: "Basic token") == nil)
  }
