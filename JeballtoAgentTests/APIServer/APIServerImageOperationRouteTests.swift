...
struct APIServerImageOperationRouteTests {
  ...
  func asyncPullFormatFailureExposesStructuredErrorCode() async throws {
    try await withTemporaryDirectory { root in
      ...
      let operation = try await seedImageOperation(
        ...
        throw ImageManagerError.unsupportedImageFormat("unversioned image")
      }
  ...
  func asyncPullTimeoutExposesStructuredErrorCode() async throws {
    try await withTemporaryDirectory { root in
      ...
      let operation = try await seedImageOperation(
        ...
        throw ImageManagerError.timeout("manifest fetch exceeded its deadline")
      }
      _ = await server.imageOperationCoordinator.wait(for: operation.id)
  ...
  func asyncPushFromVMReturnsOperationStatusAndCanBeCancelled() async throws {
    try await withTemporaryDirectory { root in
      ...
      for _ in 0 ..< 100 {
        if let status = await server.imageOperationCoordinator.status(for: operationUUID), status.state.isTerminal {
          finalStatus = status
          break
        }
  ...
  func successfulAsyncPushRoutePublishesCompletedStatusAfterFinalizing() async throws {
    try await withTemporaryDirectory { root in
      ...
      #expect(await server.imageOperationCoordinator.cancelAndWait(operationId) == false)
      try await server.vmManager.deleteVM(definition.id)
    }
  }
  ...
  func asyncPushRecordsFailureWhenRegistryPreflightFails() async throws {
    try await withTemporaryDirectory { root in
      ...
      for _ in 0 ..< 200 {
        if let status = await server.imageOperationCoordinator.status(for: operationId), status.state.isTerminal {
          terminalStatus = status
          break
        }
        try await Task.sleep(nanoseconds: 5_000_000)
