...
```
JeballtoAgent/
  APIServer/          - SimpleHTTPServer facade, request reader/decoder/router/dispatcher,
                        mutation admission gate, DTOs, and route groups
  VMManager/          - VMManager actor, VMInstance @MainActor, VMInstaller,
                        IPSWCacheLayout, IPSWSourceValidator, IPSWDownloadDelegate,
                        VMInstallationSuccessMarkerStore
  ...
  ImageManager/       - ImageManager/ImageStore actors, ImageOperationCoordinator actor,
                        VMImagePackager with VMImageBundleFormat and VMImageConfigValidator,
                        ORAS/zstd wrappers, OCI parsing, registry Keychain storage
  ...
  Common/             - Config, Logger, secrets, process helpers, ChildProcessTracker,
                        ProcessExitObserver, KeyedOperationGate, single-instance lock,
                        JeballtoCachePaths with ImageWorkSessionLock and ImageWorkChildProcessLease
...
- `APIServer` - regular CLASS. `NSLock` guards config snapshots and Jeballtofile executors. Image operation
  bookkeeping lives in `ImageOperationCoordinator`, not here. `APIMutationGate` drains ordinary mutations before
...
The version is read before the payload is decoded, so an incompatible file reports a version mismatch instead of an
incidental missing-field error. Pre-1.0 data is not migrated, and neither the incompatible file nor its `.bak`
backup is rewritten.
...
`ChildProcessTracker` drains child processes (ORAS, zstd) alongside this. `terminateAll()` sends SIGTERM to every
tracked process; `forceKillAll()` then issues a synchronous SIGKILL before the app agrees to terminate, including
when the 30-second budget is exceeded. A process is untracked only once it has actually exited, and a process
registered while shutdown is already running is signalled immediately rather than being allowed to start clean.
