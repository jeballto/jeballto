...
actor VMManager {
  ...
  private var cancellingInstallerRecoveryTokens: Set<UUID> = []
  ...
  private func recoverVMFromError(_ vmId: UUID, instance: VMInstance) async throws {
    ...
    await cancelInstallerRecovery(vmId)
  ...
  private func finishInstallerRecovery(vmId: UUID, token: UUID) {
    ...
    guard cancellingInstallerRecoveryTokens.contains(token) == false else { return }
    installerRecoveryTasks.removeValue(forKey: vmId)
    releaseInstallOperationIfIdle(vmId)
  }

  private func cancelInstallerRecovery(_ vmId: UUID) async {
    guard let recovery = installerRecoveryTasks[vmId] else {
      releaseInstallOperationIfIdle(vmId)
      return
    }
    cancellingInstallerRecoveryTokens.insert(recovery.token)
    recovery.task.cancel()
    await recovery.task.value
    if installerRecoveryTasks[vmId]?.token == recovery.token {
      installerRecoveryTasks.removeValue(forKey: vmId)
    }
    cancellingInstallerRecoveryTokens.remove(recovery.token)
    releaseInstallOperationIfIdle(vmId)
  }
  ...
  private func forceStopVMOwned(
    ...
    case .installing:
      try await forceStopInstallingVMOwned(
        vmId,
        instance: instance,
        allowTerminalInstallationFailure: allowTerminalInstallationFailure
      )
    case .error:
  ...
  private func forceStopInstallingVMOwned(
    _ vmId: UUID,
    instance: VMInstance,
    allowTerminalInstallationFailure: Bool
  ) async throws {
    let installerForRecovery = activeInstallers[vmId]
    if await cancelInstallation(vmId) {
    ...
    switch instance.currentState {
    case .error:
      try await reconcileInstallationErrorForForcedStop(
        vmId,
        instance: instance,
        allowTerminalInstallationFailure: allowTerminalInstallationFailure
      )
    case .installing:
      try await finalizeOrDiscardInstallationForForcedStop(
        vmId,
        instance: instance,
        installerForRecovery: installerForRecovery,
        allowTerminalInstallationFailure: allowTerminalInstallationFailure
      )
    default:
      break
  ...
  private func reconcileInstallationErrorForForcedStop(
    _ vmId: UUID,
    instance: VMInstance,
    allowTerminalInstallationFailure: Bool
  ) async throws {
  ...
  private func finalizeOrDiscardInstallationForForcedStop(
    _ vmId: UUID,
    instance: VMInstance,
    installerForRecovery: VMInstaller?,
    allowTerminalInstallationFailure: Bool
  ) async throws {
    var definition = await MainActor.run { instance.definition }
    let hasSuccessMarker = try VMInstallationSuccessMarkerStore.readIfPresent(for: definition) != nil
    guard hasSuccessMarker || definition.installation?.state == .finalizing else {
      try cleanupInterruptedInstallationArtifacts(for: definition)
      var installation = definition.installation
    ...
    try await completeDrainedInstallerRecoveryForForcedStop(
      vmId,
      instance: instance,
      installerForRecovery: installerForRecovery,
      allowTerminalInstallationFailure: allowTerminalInstallationFailure
  ...
  private func completeDrainedInstallerRecoveryForForcedStop(
    _ vmId: UUID,
    instance: VMInstance,
    installerForRecovery: VMInstaller?,
    allowTerminalInstallationFailure: Bool
    ...
    case .installing:
      ...
      guard hasSuccessMarker || definition.installation?.state == .finalizing else {
        throw VMManagerError.operationFailed(
          "Installation success evidence disappeared while stopping recovery for VM \(vmId)"
        )
      }
      let installer = installerForRecovery ?? VMInstaller(vmDefinition: definition, eventBus: eventBus)
      do {
    ...
    default:
      throw VMManagerError.invalidState(
        "Installer recovery for VM \(vmId) ended in unexpected state \(instance.currentState.rawValue)"
      )
    }
  }
  ...
  ///   - owningEphemeralDeletionToken: Token of an automatic deletion task that must not cancel itself during cleanup.
  func deleteVM(
    _ vmId: UUID,
    force: Bool = false,
    ...
    if let operation = vmOperations[vmId] {
      ...
      vmOperations[vmId] = "delete"
      tookOverInstallation = true
    } else {
      try claimVMOperation(vmId, operation: "delete")
      tookOverInstallation = false
    }
    defer { releaseVMOperation(vmId, operation: "delete") }
    if tookOverInstallation {
      if await cancelInstallation(vmId) {
        logInfo("Cancelled tracked installation before deleting VM \(vmId)", category: "VMManager")
      }
      await cancelInstallerRecovery(vmId)
  ...
  func setInstallerRecoveryTaskForTesting(_ task: Task<Void, Never>, vmId: UUID) throws {
    try claimVMOperation(vmId, operation: "install")
    installerRecoveryTasks[vmId] = InstallerRecoveryTaskHandle(token: UUID(), task: task)
  }

  func currentVMOperationForTesting(_ vmId: UUID) -> String? {
    vmOperations[vmId]
  }
