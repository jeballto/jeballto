...
@MainActor final class VMInstance {
  ...
  func initialize() async throws {
    ...
    vmDelegate.onError = { [weak self] error in
      Task<Void, Never> { [weak self] in
        self?.handleErrorCallback(error, from: generation)
      }
    }
    vmDelegate.onStop = { [weak self] in
      Task<Void, Never> { [weak self] in
        self?.handleStopCallback(from: generation)
      }
    }
  ...
  func stop() async throws {
    ...
    do {
      ...
      try recoverInactiveRuntimeStopOrRethrow(error, runtimeState: vm.state, forceLifecycle: false)
  ...
  func forceStopRuntime() async throws {
    ...
    do {
      ...
      try recoverInactiveRuntimeStopOrRethrow(error, runtimeState: vm.state, forceLifecycle: true)
      return
    }
  ...
  nonisolated static func runtimeIsInactiveAfterStopFailure(_ state: VZVirtualMachine.State) -> Bool {
    state == .stopped || state == .error
  }

  nonisolated static func shouldCompleteFailedStop(
    runtimeState: VZVirtualMachine.State,
    logicalState: VMState,
    forceLifecycle: Bool
  ) -> Bool {
    runtimeIsInactiveAfterStopFailure(runtimeState)
      && (forceLifecycle || logicalState == .stopping || logicalState == .error || logicalState == .stopped)
  }

  func recoverInactiveRuntimeStopOrRethrow(
    _ stopError: Error,
    runtimeState: VZVirtualMachine.State,
    forceLifecycle: Bool
  ) throws {
    let logicalState = stateMachine.currentState
    guard Self.shouldCompleteFailedStop(
      runtimeState: runtimeState,
      logicalState: logicalState,
      forceLifecycle: forceLifecycle
    ) else {
      let operation = forceLifecycle ? "Forced stop" : "Stop"
      logError("\(operation) failed for VM \(definition.id): \(stopError)", category: "VMInstance")
      recordFailureIfNeeded(stopError)
      throw stopError
    }

    logWarning(
      "VM \(definition.id) stop reported \(stopError), but the runtime is inactive "
        + "(VZ:\(runtimeState.rawValue)); completing the requested stop",
      category: "VMInstance"
    )
    ...
    do {
      ...
      try removeSavedStateIfPresent(context: "after inactive runtime stop recovery")
    } catch {
      logError(
        "Failed to finalize inactive runtime stop for VM \(definition.id): \(error)",
        category: "VMInstance"
      )
      recordFailureIfNeeded(error)
      throw error
  ...
  func handleErrorCallback(_ error: Error, from generation: UUID) {
    guard generation == runtimeGeneration else {
      logInfo("Ignoring error callback from retired runtime for VM \(definition.id)", category: "VMInstance")
      return
    }
    handleError(error)
  }
  ...
  func handleStopCallback(from generation: UUID) {
    guard generation == runtimeGeneration else {
      logInfo("Ignoring stop callback from retired runtime for VM \(definition.id)", category: "VMInstance")
      return
    }
    handleStop()
  }
