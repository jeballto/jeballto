...
### ImageOperationCoordinator (actor)

Single owner of everything that tracks an asynchronous image pull or push. `ImageManager` owns the image
directory, the index, registry auth and source reservations; the coordinator owns the *operation*.
...
|---|---:|---|
| VM database | v2 | Top-level `version` in `vms.json` |
| Local image index | v2 | Top-level `version` in `images.json` |
...
The version is read before the payload is decoded, so a database or index written by an older or newer agent
reports an explicit version mismatch rather than an incidental missing-field error. Pre-1.0 databases and indexes
are not migrated: neither the incompatible file nor its `.bak` backup is rewritten, so the operator can inspect or
archive them before removing them by hand.

## State Ownership

Every piece of shared mutable state has exactly one owner. Helper types do work on behalf of an owner but never
hold durable state independently, so there is no second place a value can be updated from.

| State | Owner | Notes |
|---|---|---|
| VM registry, capacity reservations, operation claims, VM lifecycle | `VMManager` | Persists through `PersistenceStore` and publishes events only after the durable write |
| VM runtime | `VMInstance` (@MainActor) | Retired runtimes are fenced by generation, so a stale callback cannot write back |
| Image directory, index commits, registry auth, reference gates, source reservations | `ImageManager` | The authoritative index commit stays here even for work a pipeline prepared |
| Image operation admission, tasks, status, cancellation, retention | `ImageOperationCoordinator` | See above |
| Port and NAT allocation | `NetworkManager`, `PortForwardingManager` | |
| Child process lifetime | `ChildProcessTracker` | Untracks only after real exit |
| Persisted VM records | `PersistenceStore` | |

Locks are used where a framework callback demands synchronous state: Network.framework connection handlers,
`Process` termination handlers, and file-descriptor bookkeeping. These are deliberate and are not converted to
actors, because the callback cannot await.
