...
Each cache entry keeps the restore image's original filename inside a source-specific directory, for example:

```
IPSWCache/
  UniversalMac_26.3.1_25D2128_Restore--<source-id>/
    UniversalMac_26.3.1_25D2128_Restore.ipsw
```

The source ID is derived from the complete download URL. It prevents different URLs with the same filename from
sharing the wrong restore image. Existing `restore-<source-id>.ipsw` cache entries created by earlier builds are
moved into the readable layout when that source is used again, without downloading the IPSW again.
...
Active image operations are drained in the same window: new pulls and pushes are refused, in-flight ones are
cancelled, and the agent waits for their tasks. Child processes started by those operations (`oras`, `zstd`) are
sent SIGTERM during the drain and SIGKILL immediately before the agent exits, including when the 30-second budget
is exceeded. A stubborn child therefore cannot outlive the agent and keep holding an image work session.
