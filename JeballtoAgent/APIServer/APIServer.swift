...
final class APIServer: @unchecked Sendable {
  ...
  let imageOperationCoordinator = ImageOperationCoordinator()
  ...
  func stop() async {
    ...
    _ = await imageOperationCoordinator.closeAdmissionsAndDrain()
  ...
  func endExclusiveMaintenance() async {
    await imageOperationCoordinator.resumeAdmissions()
  ...
  func resumeImageOperationTaskRegistrationForTesting() async {
    await imageOperationCoordinator.resumeAdmissions()
  }
  #endif
  ...
  func cancelAndWaitAllBackgroundTasks() async {
    ...
    let images = await imageOperationCoordinator.cancelAll().count
