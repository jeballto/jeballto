...
/// Safe subset of configuration (excludes tokens, file paths)
...
struct UpdateConfigRequest: Codable {
  ...
  func validate(currentConfig: NetworkingConfig? = nil) -> (valid: Bool, error: String?) {
    if let logging {
      if let level = logging.level, !Self.validLogLevels.contains(level) {
        return (false, "Invalid log level. Must be one of: \(Self.validLogLevels.joined(separator: ", "))")
      }
      if let retentionDays = logging.retentionDays, retentionDays < 1 {
        return (false, "retentionDays must be at least 1")
      }
      if let maxTotalSize = logging.maxTotalSize {
        guard let bytes = LoggingConfig.parseSize(maxTotalSize) else {
          return (false, "maxTotalSize must be a value with unit, e.g. \"500MB\" or \"2GB\"")
        }
        if bytes < 1_048_576 {
          return (false, "maxTotalSize must be at least 1MB")
        }
      }
      if let tz = logging.timezone, TimeZone(identifier: tz) == nil {
        return (
          false,
          "Invalid timezone identifier '\(tz)'. Use an IANA timezone name (e.g. 'UTC', 'America/New_York')."
        )
      }
    }
    if let networking {
      if let start = networking.sshPortRangeStart, start < 1024 || start > 65535 {
        return (false, "SSH port range start must be 1024-65535")
      }
      if let end = networking.sshPortRangeEnd, end < 1024 || end > 65535 {
        return (false, "SSH port range end must be 1024-65535")
      }
      // Validate merged SSH range: use provided values or fall back to current config
      if let currentConfig {
        let effectiveStart = networking.sshPortRangeStart ?? currentConfig.sshPortRangeStart
        let effectiveEnd = networking.sshPortRangeEnd ?? currentConfig.sshPortRangeEnd
        if effectiveStart > effectiveEnd {
          return (false, "SSH port range start must not exceed end")
        }
      } else if let start = networking.sshPortRangeStart,
                let end = networking.sshPortRangeEnd,
                start > end
      {
        return (false, "SSH port range start must not exceed end")
      }

      // VNC port range validation
      if let start = networking.vncPortRangeStart, start < 1024 || start > 65535 {
        return (false, "VNC port range start must be 1024-65535")
      }
      if let end = networking.vncPortRangeEnd, end < 1024 || end > 65535 {
        return (false, "VNC port range end must be 1024-65535")
      }
      // Validate merged VNC range
      if let currentConfig {
        let effectiveStart = networking.vncPortRangeStart ?? currentConfig.vncPortRangeStart
        let effectiveEnd = networking.vncPortRangeEnd ?? currentConfig.vncPortRangeEnd
        if effectiveStart > effectiveEnd {
          return (false, "VNC port range start must not exceed end")
        }
      } else if let start = networking.vncPortRangeStart,
                let end = networking.vncPortRangeEnd,
                start > end
      {
        return (false, "VNC port range start must not exceed end")
      }
