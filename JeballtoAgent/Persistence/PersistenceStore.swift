...
enum PersistenceError: Error, LocalizedError {
  ...
  case missingDatabaseVersion(path: String, expected: Int)
  case invalidDatabaseVersion(path: String, expected: Int)
  ...
  /// local database predates 1.0. Neither the database nor its backup is rewritten, so recovery is manual.
  private static let versionRecoveryHint =
    "Pre-1.0 VM database migration is not supported. Stop the agent, remove the incompatible database "
      + "and its .bak backup, then recreate the VMs"

  var errorDescription: String? {
    switch self {
    case .fileNotFound(let path): "File not found at path: \(path)"
    case .invalidData(let reason): "Invalid data: \(reason)"
    case .missingDatabaseVersion(let path, let expected):
      "VM database at \(path) has no version, expected \(expected). \(Self.versionRecoveryHint)"
    case .invalidDatabaseVersion(let path, let expected):
      "VM database version at \(path) must be an integer, expected \(expected). \(Self.versionRecoveryHint)"
    case .unsupportedDatabaseVersion(let found, let expected):
      "Unsupported VM database version \(found), expected \(expected). \(Self.versionRecoveryHint)"
...
private struct VMDatabaseVersionProbe: Decodable {
  let version: Int
}
...
actor PersistenceStore {
  ...
  func validateLoaded() throws {
    ensureLoaded()
    if let loadFailure {
      if Self.isDatabaseVersionError(loadFailure) {
        throw loadFailure
      }
  ...
  private func loadFromDisk() throws -> VMDatabase {
    ...
    do {
      ...
      if Self.isDatabaseVersionError(primaryError) {
        throw primaryError
      }
      ...
      do {
        ...
        if Self.isDatabaseVersionError(error) {
          throw error
        }
        throw PersistenceError.invalidData(
  ...
  private func decodeDatabase(at path: String) throws -> VMDatabase {
    do {
      ...
      try validateDatabaseVersion(in: data, at: path)
  ...
  private func validateDatabase(_ candidate: VMDatabase) throws {
    guard candidate.version == VMDatabase.currentVersion else {
      throw PersistenceError.unsupportedDatabaseVersion(
        found: candidate.version,
        expected: VMDatabase.currentVersion
      )
    }
  ...
  private func validateDatabaseVersion(in data: Data, at path: String) throws {
    let version: Int
    do {
      version = try decoder.decode(VMDatabaseVersionProbe.self, from: data).version
    } catch DecodingError.keyNotFound(let key, _) where key.stringValue == "version" {
      throw PersistenceError.missingDatabaseVersion(
        path: path,
        expected: VMDatabase.currentVersion
      )
    } catch DecodingError.typeMismatch(_, let context)
      where context.codingPath.last?.stringValue == "version" {
        throw PersistenceError.invalidDatabaseVersion(
          path: path,
          expected: VMDatabase.currentVersion
        )
    ...
    guard version == VMDatabase.currentVersion else {
      throw PersistenceError.unsupportedDatabaseVersion(
        found: version,
        expected: VMDatabase.currentVersion
      )
  ...
  private static func isDatabaseVersionError(_ error: Error) -> Bool {
    guard let persistenceError = error as? PersistenceError else { return false }
    switch persistenceError {
    case .missingDatabaseVersion, .invalidDatabaseVersion, .unsupportedDatabaseVersion:
      return true
    default:
      return false
    }
