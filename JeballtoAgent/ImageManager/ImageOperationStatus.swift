...
enum ImageOperationStatusReducer {
  ...
  static func update(_ status: inout ImageOperationStatus, update: ImageOperationProgressUpdate) {
    guard status.state.isTerminal == false else { return }
  ...
  static func complete(_ status: inout ImageOperationStatus, record: ImageRecord) {
    guard status.state.isTerminal == false else { return }
    let now = Date()
    let uptime = ProcessInfo.processInfo.systemUptime
  ...
  static func fail(_ status: inout ImageOperationStatus, error: Error) {
    guard status.state.isTerminal == false else { return }
    let now = Date()
    let uptime = ProcessInfo.processInfo.systemUptime
  ...
  static func requestCancellation(_ status: inout ImageOperationStatus) {
    guard status.state.isTerminal == false else { return }
    let now = Date()
    let uptime = ProcessInfo.processInfo.systemUptime
  ...
  private static func clampedActiveProgress(_ value: Double) -> Double {
  ...
  private static func adding(_ delta: UInt64, to value: UInt64) -> UInt64 {
    let (result, overflow) = value.addingReportingOverflow(delta)
    return overflow ? UInt64.max : result
  }

  private static func computedProgress(for status: ImageOperationStatus) -> Double? {
  ...
  private static func weightedProgress(stage: ImageOperationStage, stageProgress: Double) -> Double {
    switch stage {
    case .compressing:
