...
struct ImageIndex: Codable, Sendable {
  ...
  init(version: Int = ImageIndex.currentVersion, images: [UUID: ImageRecord] = [:]) {
    self.version = version
    self.images = images
  }

  static var empty: ImageIndex { ImageIndex() }
}
...
enum ImageStoreError: Error, LocalizedError {
  ...
  case missingIndexVersion(path: String, expected: Int)
  case invalidIndexVersion(path: String, expected: Int)
  ...
  /// The version gate runs before decoding, so these errors are the operator's first signal that a
  /// local index predates 1.0. Neither the index nor its backup is rewritten, so recovery is manual.
  private static let versionRecoveryHint =
    "Pre-1.0 image index migration is not supported. Stop the agent, remove the incompatible index "
      + "and its .bak backup along with the managed image bundles it referenced, then pull or push again"
  ...
  var errorDescription: String? {
    ...
    case .missingIndexVersion(let path, let expected):
      "Image index at \(path) has no version, expected \(expected). \(Self.versionRecoveryHint)"
    case .invalidIndexVersion(let path, let expected):
      "Image index version at \(path) must be an integer, expected \(expected). \(Self.versionRecoveryHint)"
    case .unsupportedIndexVersion(let path, let actual, let expected):
      "Unsupported image index version \(actual) at \(path), expected \(expected). \(Self.versionRecoveryHint)"
...
actor ImageStore {
  ...
  ///   - durablePostPublishSync: Optional durability hook used after atomically publishing the image index.
  init(
    storagePath: String,
  ...
  private func ensureLoaded() throws {
    ...
    if let loadFailure {
      if Self.isIndexVersionError(loadFailure) {
        throw loadFailure
      }
  ...
  private func loadFromDisk() throws -> ImageIndex {
    ...
    do {
      ...
      if Self.isIndexVersionError(error) {
        throw error
      }
  ...
  private func decodeIndex(at path: String) throws -> ImageIndex {
    do {
      ...
      try validateIndexVersion(in: data, at: path)
  ...
  private func writeToDisk(_ candidate: ImageIndex) throws {
    ...
    do {
      ...
      if fileManager.fileExists(atPath: indexPath) {
        try refuseToOverwriteIncompatibleBackup(at: backupPath)
  ...
  private func validateIndexVersion(in data: Data, at path: String) throws {
    let version: Int
    do {
      version = try decoder.decode(ImageIndexVersionProbe.self, from: data).version
    } catch DecodingError.keyNotFound(let key, _) where key.stringValue == "version" {
      throw ImageStoreError.missingIndexVersion(
        path: path,
        expected: ImageIndex.currentVersion
      )
    } catch DecodingError.typeMismatch(_, let context)
      where context.codingPath.last?.stringValue == "version" {
        throw ImageStoreError.invalidIndexVersion(
          path: path,
          expected: ImageIndex.currentVersion
        )
    ...
    guard version == ImageIndex.currentVersion else {
  ...
  private func refuseToOverwriteIncompatibleBackup(at path: String) throws {
    guard Self.filesystemEntryExists(at: path) else { return }
    do {
      let data = try boundedIndexData(at: path)
      try validateIndexVersion(in: data, at: path)
    } catch {
  ...
  private func validate(_ decoded: ImageIndex) throws {
    guard decoded.version == ImageIndex.currentVersion else {
      throw ImageStoreError.unsupportedIndexVersion(
        path: indexPath,
  ...
  private static func isIndexVersionError(_ error: Error) -> Bool {
    guard let imageStoreError = error as? ImageStoreError else { return false }
    switch imageStoreError {
    case .missingIndexVersion, .invalidIndexVersion, .unsupportedIndexVersion:
      return true
    default:
      return false
    }
